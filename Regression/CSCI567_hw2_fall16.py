...
def plot_histogram(data_frame):
    for column in data_frame.columns:
        ...
        plt.title(column)
        fig1 = plt.gcf()
        fig1.savefig(column+'.png', dpi=100)
        plt.show()

    print '\nHistograms are also saved in the current folder.'
...
def get_train_test_standardized_df(boston):
    ...
    for i in range(0, len(boston.data)):
        temp_list = [1]
        temp_list.extend(boston.data[i])
        if i % 7 == 0:
    ...
    columns = ['x_0']
    columns.extend(boston.feature_names)
    train_data = pandas.DataFrame(train_list, columns=columns)
    test_data = pandas.DataFrame(test_list, columns=columns)

    print '\n************************* Histograms *********************************\n'
    plot_histogram(train_data[boston.feature_names])
    ...
    standardize_train_columns(train_data, test_data, columns[1:])
    # Should we standardize ?
    # mean, std, train_target = standardize_target_train(train_target)
    # test_target = standardize_target_test(mean, std, test_target)
...
def compute_result(data_frame, theta):
    return theta.dot(data_frame.transpose())
...
def compute_mse(target, results):
    ...
    loss = 0.0
    for i in range(0,N):
        loss += math.pow((target[i] - results[i]), 2)
    return loss / N
...
def cross_validation(train_data):
    ...
    for lambda_value in lambda_array:
        ...
        while iteration < 10:
            ...
            cv_target_train = numpy.concatenate(k_fold_target[:iteration]+k_fold_target[iteration+1:])
        ...
        writer.writerow([round(lambda_value, 4), average])
        if lambda_value in [1.0001, 2.0001, 3.0001, 4.0001, 5.0001, 6.0001, 7.0001, 8.0001, 9.0001]:
            print 'Lambda: ' + str(lambda_value) + '\t 10-fold Avg MSE: ' + str(average)
...
def get_list(train_data, train_target, which_list):
    if which_list == "top_4_pearson":
        list_pearson = []
        for column in train_data.columns:
            tuple_value = (column, abs(float(pearson(train_data[column], train_target))))
            list_pearson.append(tuple_value)

        list_top_4 = sorted(list_pearson, key=lambda x: x[1], reverse=True)[:4]
        top_features = []
        for item in list_top_4:
            top_features.append(item[0])

        print 'Top 4 correlated features to target: ' + str(top_features)
        list_column = ['x_0']
        for entry in list_top_4:
            list_column.append(entry[0])
        return [list_column]
...
def get_residue_based_mse(train_data, train_target, test_data, test_target):
    ...
    for i in xrange(0,4):
        ...
        residue = get_residue(train_target, result_train)
        target = residue
    print 'Top 4 features for Residue based selection: '+ str(features)
    return [mse_train, mse_test]
...
if __name__ == "__main__":
    ...
    for lambda_value in lambda_array:
        ...
        print 'Train result for lambda: ' + str(lambda_value) + ' is ' + str(mse_train)
        print 'Test result for lambda: ' + str(lambda_value) + ' is ' + str(mse_test) + '\n'
    ...
    print '\nAll the values of LAMBDA and corresponding MSE are written to \'results.csv\' in the same folder..'
    print '\nLowest Train MSE is %.9f\t lambda %.4f '%(result[1], result[0])
    [mse_train, mse_test] = ridge_regression(train_data, train_target, test_data, test_target, result[0])
    print '\nLowest TEST MSE is %.9f\t lambda %.4f ' % (mse_test, result[0])
    ...
    for tuple_value in train_test_mse_list:
        print 'Corresponding  Train MSE: ' + str(tuple_value[0][0])
        print 'Corresponding Test MSE: ' + str(tuple_value[0][1])+ '\n'

    [mse_train, mse_test] = get_residue_based_mse(train_data, train_target, test_data, test_target)
    print 'Corresponding Train MSE: ' + str(mse_train)
    print 'Corresponding Test MSE: ' + str(mse_test)+'\n'
    ...
    print 'Top 4 features after Brute force selection: '+ str(min_train_list[1:])
    [result_train, result_test, mse_train, mse_test] = linear_regression(train_data[min_train_list], train_target, test_data[min_train_list], test_target)
