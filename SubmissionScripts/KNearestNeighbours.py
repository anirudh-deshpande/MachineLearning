...
def get_dicts(train_csv, test_csv, training):
    ...
    total = 0
    if training:
        total = len(test_csv) - 1
    else:
        total = len(test_csv)

    print "Manhatten Accuracy(L1) for K = 1 : %.4f" % (float(correct_manhatten_1) * 100.0/ total)
    print "Eucledian Accuracy(L2) for K = 1 : %.4f" % (float(correct_euclidean_1) * 100.0/ total)

    print "\nManhatten Accuracy(L1) for K = 3 : %.4f" % (float(correct_manhatten_3) * 100.0/ total)
    print "Eucledian Accuracy(L2) for K = 3 : %.4f" % (float(correct_euclidean_3) * 100.0/ total)

    print "\nManhatten Accuracy(L1) for K = 5 : %.4f" % (float(correct_manhatten_5) * 100.0/ total)
    print "Eucledian Accuracy(L2) for K = 5 : %.4f" % (float(correct_euclidean_5) * 100.0/ total)

    print "\nManhatten Accuracy(L1) for K = 7 : %.4f" % (float(correct_manhatten_7) * 100.0/ total)
